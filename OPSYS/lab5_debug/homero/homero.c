...
void fiu_feladat(int pipew, char *name)
{
    ...
    {
        if (write(pipew, (void *)&x, sizeof(double)) == -1)
            syserr("write");
    }
    if (ferror(fp1))
        syserr("fp1err");
...
int main(int argc, char const *argv[])
{
    ...
    else
    {
        ...
        else
        {
            ...
            // A folyamat feladata
            FILE *fifo;
            if ((fifo = fopen("myfifo.txt", "w")) == NULL)
                syserr("fifo");
            ...
            while (1)
            {
                if ((n1 = read(pfd1[0], (void *)&x1, sizeof(double))) == -1)
                    syserr("read");
                if ((n2 = read(pfd2[0], (void *)&x2, sizeof(double))) == -1)
                    syserr("read");
                if (n1 == 0 || n2 == 0)
                    break;
                if (fprintf(fifo, "%lf\n", x1 - x2) < 0)
